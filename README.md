...
### 2. 개발 서버 — 원클릭 (start = 켜기 / stop = 끄기)

명령어 칠 필요 없이 **파일 더블클릭**:
- **Windows**: `start.bat` 더블클릭 → 서버 뜨고 브라우저 자동 열림 / 끌 때 `stop.bat`
- **Mac**: `start.command` 더블클릭 (처음 한 번은 우클릭 → 열기) / 끌 때 `stop.command`
- 첫 실행이 곧 환경 셋업(의존성 설치)이라 몇 분 걸리고, 그다음부턴 빠르다.
