...
function regionOptions() {
  return Array.from(new Set(jobs.map((job) => job.region.split(" ")[0]))).sort((a, b) => a.localeCompare(b, "ko-KR"));
}
...
export default function JobsPage({ searchParams = {} }: JobsPageProps) {
  const desktopPage = queryJobPostings(jobs, searchParams, 20);
  const mobilePage = queryJobPostings(jobs, searchParams, 10);
    ...
    <div className="lg:flex lg:gap-6">
      ...
      <div className="min-w-0 flex-1 space-y-5">
        <div className="flex items-center justify-between gap-3">
          ...
          <Link
            href="/jobs/search"
            className="inline-flex h-10 items-center gap-2 rounded-md border border-line bg-surface px-3 text-sm font-semibold text-ink shadow-card lg:hidden"
          >
        ...
        <div className="hidden lg:block">
          <JobFilterBox searchParams={searchParams} regions={regionOptions()} />
        </div>
        ...
        <div className="hidden overflow-hidden rounded-md border border-line bg-surface shadow-card lg:block">
          ...
          {desktopPage.items.length > 0 ? (
            desktopPage.items.map((job) => <JobRow key={job.id} job={job} />)
          ) : (
            <EmptyState title="조건에 맞는 모집 공고가 없습니다." />
          )}
        </div>
        ...
        <div className="space-y-5 lg:hidden">
          ...
          <section>
            ...
            <div className="grid gap-3">
              {(normalMobile.length > 0 ? normalMobile : mobilePage.items).map((job) => (
                <JobCard key={job.id} job={job} compact />
              ))}
            </div>
          </section>
