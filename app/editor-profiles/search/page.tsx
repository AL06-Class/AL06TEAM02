    ...
    <form action="/editor-profiles" className="mx-auto max-w-2xl pb-20">
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">편집 분야, 편집 가능 툴, 경력과 희망 단가를 선택하세요.</p>
        </div>
        ...
        <section className="rounded-md border border-line bg-surface p-4 shadow-card">
          ...
          <div className="grid gap-2">
            {EDITING_TOOL_OPTIONS.map((tool) => <Checkbox key={tool} name="equipment" value={tool} label={tool} />)}
          </div>
        </section>

        <Input name="q" label="키워드" search placeholder="이름, 분야, 편집 가능 툴, 소개" />
      </div>
