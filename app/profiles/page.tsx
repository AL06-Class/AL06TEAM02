...
function regionOptions() {
  return Array.from(new Set(profiles.map((profile) => profile.region.split(" ")[0]))).sort((a, b) => a.localeCompare(b, "ko-KR"));
}
...
export default function ProfilesPage({ searchParams = {} }: ProfilesPageProps) {
  const desktopPage = queryShooterProfiles(profiles, searchParams, 12);
  const mobilePage = queryShooterProfiles(profiles, searchParams, 10);
  ...
  return (
    <div className="lg:flex lg:gap-6">
      ...
      <div className="min-w-0 flex-1 space-y-5">
        ...
        <div className="hidden lg:block">
          <ProfileFilterBox searchParams={searchParams} regions={regionOptions()} />
        </div>
        ...
        <div className="hidden lg:grid lg:grid-cols-3 lg:gap-4">
          {desktopPage.items.map((profile) => (
            <ProfileCard key={profile.id} profile={profile} />
          ))}
        </div>

        <div className="grid grid-cols-2 gap-3 lg:hidden">
          {mobilePage.items.map((profile) => (
